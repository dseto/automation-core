param(
    [string]$SessionExample = "ui-tests/artifacts/recorder/session.json",
    [string]$UiMap = "ui-tests/ui/ui-map.yaml",
    [string]$OutputDir = "ui-tests/artifacts/semantic-resolution-e2e",
    [string]$ScenarioName = ""
)
...
if ([string]::IsNullOrWhiteSpace($ScenarioName)) { $scenarioArg = "" } else { $scenarioArg = " --scenario `"$ScenarioName`"" }
$cmd = "dotnet run --project src/Automation.RecorderTool generate-draft --input `"$sessionPath`" --output `"$outDir`"$scenarioArg"
Write-Host $cmd
$exit = & dotnet run --project src/Automation.RecorderTool generate-draft --input "$sessionPath" --output "$outDir"$scenarioArg
