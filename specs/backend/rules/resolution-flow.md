# Resolution Flow (Normativo)

> Base: `_legacy/04-runtime-resolution.md` + implementação em `Automation.Core/DataMap/*` e `Automation.Core/*Resolver*`.

## Ordem (alto nível)
1. Determinar **PageContext** (página atual) ou página explícita no ref.
2. Resolver elemento no **UiMapModel** (pages/modals).
3. Converter `test_id` em locator CSS (`[data-testid='...']`).
4. Resolver dados (se aplicável) no **DataResolver**.
5. Executar step com waits e evidência (se habilitado).
