...
> Base: `_legacy/06-escape-hatch.md`

## Objetivo
Permitir bypass controlado quando:
- o app sob teste não possui `data-testid` adequado
- a UI é altamente dinâmica e não mapeável a tempo
- é necessário um seletor específico temporário

## Regras
- Toda usage deve ser rastreável (log/relatório).
- Preferir TODO explícito no spec (com prazo/owner) e registro no backlog.
- Escape hatch não pode virar padrão: exige justificativa.
